...
//	debug = par("debug").boolValue();
	...
	messageDelay = registerSignal("messageDelay");
	messageOverhead = registerSignal("messageOverhead");
	messageOverheadRelative = registerSignal("messageOverheadRelative");
	messageDelivery = registerSignal("messageDelivery");

	currentBroadCastTimeOut = new cMessage("currentBroadCastTimeOut");

	MYDEBUG << "initialized TraCIScenarioManager" << endl;
}

void TraCIScenarioManager::vehicleCreateMessage(string vehicleID, int messageID, Coord messageRegionCenter, int messageRegionRadius, simtime_t timestamp) {
    pair<simtime_t, map<string, pair<simtime_t, bool> > > messageEntry;
    messageEntry.first = timestamp;
    messageEntry.second[vehicleID] = make_pair(timestamp, true);
    messagesTable[messageID] = messageEntry;
    currentMessageID = messageID;
    regionCenter = messageRegionCenter;
    regionRadius = messageRegionRadius;

    map<string, Coord>  allVehiclePositions;
    for(std::map<std::string, cModule*>::iterator it = hosts.begin(); it != hosts.end(); it++){
        cModule* module = it->second;
        if (module->findSubmodule("veinsmobility") != -1) {
            TraCIMobility *vehicleMobility = dynamic_cast<TraCIMobility*>(module->getSubmodule("veinsmobility"));
            allVehiclePositions[it->first] = vehicleMobility->getCurrentPosition();
        }
    }
    allVehiclesInRegion.clear();
    for(map<string, Coord>::iterator it = allVehiclePositions.begin(); it != allVehiclePositions.end(); ++it) {
        if (it->second.distance(regionCenter) <= regionRadius) {
            allVehiclesInRegion.insert(it->first);
        }
    }
    if(debug) std::cout << "Possui um total de " << allVehiclePositions.size() << " veiculos e " << allVehiclesInRegion.size() << " veiculos na regiao" << endl;

    scheduleAt(simTime() + 4.0, currentBroadCastTimeOut);

}
void TraCIScenarioManager::vehicleReceivedMessage(string vehicleID, int messageID, simtime_t timestamp, bool forward) {
    if (messageID == currentMessageID && messagesTable.count(messageID) > 0) {
        messagesTable[messageID].second[vehicleID] = make_pair(timestamp, forward);
    }
}
void TraCIScenarioManager::messageTimeOut(int messageID) {
    currentMessageID = -1;
    pair<simtime_t, map<string, pair<simtime_t, bool> > > messageEntry = messagesTable[messageID];


    int vehicleCount = 0, vehiclesInRegion = (int)allVehiclesInRegion.size();
    int forwardersCount = 0;
    double delaySum = 0;
    string vehiclesStr, allVehiclesInRegionStr, forwardersStr;
    for(set<string>::iterator it = allVehiclesInRegion.begin(); it != allVehiclesInRegion.end(); ++it) allVehiclesInRegionStr += *it + " ";
    for(map<string, pair<simtime_t, bool> >::iterator it = messageEntry.second.begin(); it != messageEntry.second.end(); ++it) {
        if (allVehiclesInRegion.count(it->first) == 0) continue;
        vehicleCount++;
        vehiclesStr += it->first + " ";
        delaySum += (it->second.first - messageEntry.first).dbl();
        if (it->second.second) {
            forwardersCount++;
            forwardersStr += it->first + " ";
        }
    }

    if(debug) {
        std::cout << "Lista de veiculos na regiao: " << allVehiclesInRegionStr << endl;
        std::cout << "Lista de veiculos que receberam: " << vehiclesStr << endl;
        std::cout << "Lista de veiculos que retransmitiram: " << forwardersStr << endl;
        std::cout << "Total de Veiculos na regiao: " << vehiclesInRegion << endl;
        std::cout << "Total de Veiculos que receberam na regiao: " << vehicleCount << endl;
        std::cout << "Total de Veiculos que retransmitiram: " << forwardersCount << endl;
        std::cout << "Soma dos atrasos: " << delaySum << endl;
    }


    emit(messageDelay, vehicleCount == 0 ? 0 : delaySum / vehicleCount);
    emit(messageOverhead, forwardersCount);
    emit(messageOverheadRelative, vehiclesInRegion == 0 ? 0 : (double)forwardersCount / vehiclesInRegion);
    emit(messageDelivery, vehiclesInRegion == 0 ? 1 : (double)vehicleCount / vehiclesInRegion);
}
...
void TraCIScenarioManager::handleSelfMsg(cMessage *msg) {
    if (msg == currentBroadCastTimeOut) {
        messageTimeOut(currentMessageID);
        return;
    }
