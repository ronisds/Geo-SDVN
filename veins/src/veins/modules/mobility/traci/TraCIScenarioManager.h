...
class TraCIScenarioManager : public cSimpleModule
{
	public:
		...
		//  MessageID - Creation Time - VehicleID - Received Time - Was Forwarded
		map<int, pair<simtime_t, map<string, pair<simtime_t, bool> > > > messagesTable;
		int currentMessageID;
		Coord regionCenter;
		int regionRadius;
		set<string> allVehiclesInRegion;
		cMessage* currentBroadCastTimeOut;


		void vehicleCreateMessage(string vehicleID, int messageID, Coord regionCenter, int regionRadius, simtime_t timestamp);
		void vehicleReceivedMessage(string vehicleID, int messageID, simtime_t timestamp, bool forward);
		void messageTimeOut(int messageID);

		simsignal_t messageDelay;
		simsignal_t messageOverhead;
		simsignal_t messageOverheadRelative;
		simsignal_t messageDelivery;
